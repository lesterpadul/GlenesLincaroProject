	...
	jQuery(function(){
		jQuery( "#search" ).autocomplete({
				...
				source: function( request, response ) {     
					jQuery.getJSON( "http://localhost/GlenesLincaroProject/Users/search", {     //the request is done to the source file declared here
						term: request.term
					}, response );
				},
				...
				select: function( event, ui ) {           //this is done, after the request is complete
					//alert(ui.item.id);
					//passValue(ui.item.id);  
					jQuery('#message-to').val(ui.item.id);					//call some function to do something with the selected itemz(basing on its id)
